...
int *reversed_subtree_array(bstNode *head, int find_key, int *num_elements);
...
int *reversed_subtree_array(bstNode *head, int find_key, int *num_elements)
{
  if(is_member(head,find_key)==false)
  {
    num_elements=0;
    return NULL;
  }
  bstNode *finder=head;
  while(finder->key!=find_key)
  {
    if(finder->key>find_key)
    {
  ...
  int *array=malloc(sizeof(int)*num_nodes(finder));
  num_elements=num_nodes(finder);
  int i=0;
  while(finder!=NULL)
  {
    if(find_maximum(finder)==find_key)
    {
      array[i]=find_key;
      finder=finder->left_child;
    }
    ...
    {
      array[i]=find_maximum(finder);
      delete(finder,array[i],NULL);
    }
    i++;
  }
  return array;
